...
it("boolean", () => {
  ...
  expect(result.parameters).toEqual([
    { name: "0", value: { booleanValue: true } },
  ]);
});
...
it("object (Array (bigint))", () => {
  const result = db
    .selectFrom("person")
    .selectAll()
    .where("score", "in", [BigInt(10), BigInt(100), BigInt(1000)])
    .compile();
  expect(result.sql).toEqual(
    `select * from "person" where "score" in (:0, :1, :2)`
  );
  expect(result.parameters).toEqual([
    {
      name: "0",
...
it("object (Array (boolean))", () => {
  const result = db
    ...
    .compile();
  expect(result.sql).toEqual(
    `select * from "person" where "is_active" in (:0, :1)`
  );
  expect(result.parameters).toEqual([
    {
      name: "0",
...
it("object (Array (number))", () => {
  const result = db
    ...
    .compile();
  expect(result.sql).toEqual(
    `select * from "person" where "id" in (:0, :1, :2)`
  );
  expect(result.parameters).toEqual([
    {
      name: "0",
...
it("object (Array (string))", () => {
  const result = db
    .selectFrom("person")
    .selectAll()
    .where(sql`lower(first_name)`, "in", ["alice", "bob", "carol"])
    .compile();
  expect(result.sql).toEqual(
    `select * from "person" where lower(first_name) in (:0, :1, :2)`
  );
  expect(result.parameters).toEqual([
    {
      name: "0",
...
it("object (Buffer)", () => {
  ...
  expect(result.parameters).toEqual([
    { name: "0", value: { blobValue: Buffer.from("abc") } },
  ]);
});
  ...
  expect(result.parameters).toEqual([
    {
      name: "0",
      typeHint: "TIMESTAMP",
      value: { stringValue: "2022-10-20 12:34:56.789" },
    },
  ]);
});
...
it("object (value object (arrays))", () => {
  ...
  for (const [key, value] of Object.entries(inputs)) {
    ...
    expect(result.parameters).toEqual([
      { name: "0", value: { arrayValue: { [key]: value } } },
    ]);
  }
});
...
it("object (value object (DATE))", () => {
  ...
  for (const [stringValue, fixedStringValue] of pairs) {
    const result = db
      ...
      .compile(); // eslint-disable-line @typescript-eslint/no-explicit-any
    expect(result.sql).toEqual(
      `select * from "person" where "created_at" < :0`
    );
    expect(result.parameters).toEqual([
      { name: "0", typeHint: "DATE", value: { stringValue: fixedStringValue } },
    ]);
  }
});
...
it("object (value object (DECIMAL))", () => {
  ...
  expect(result.parameters).toEqual([
    { name: "0", typeHint: "DECIMAL", value: { stringValue: "1000" } },
  ]);
});
...
it("object (value object (TIME))", () => {
  ...
  for (const [stringValue, fixedStringValue] of pairs) {
    ...
    expect(result.parameters).toEqual([
      { name: "0", typeHint: "TIME", value: { stringValue: fixedStringValue } },
    ]);
  }
});
    ...
    expect(result.parameters).toEqual([
      {
        name: "0",
        typeHint: "TIMESTAMP",
        value: { stringValue: fixedStringValue },
      },
    ]);
  }
});
...
it("string", () => {
  const result = db
    ...
    .compile();
  expect(result.sql).toEqual(
    `select * from "person" where "first_name" ilike :0`
  );
  expect(result.parameters).toEqual([
    { name: "0", value: { stringValue: "%john%" } },
  ]);
});
